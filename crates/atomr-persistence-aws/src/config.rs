...
impl DynamoConfig {
    ...
    /// Env lookup: `ATOMR_PERSISTENCE_DYNAMO_TABLE`, endpoint override
    /// via `ATOMR_PERSISTENCE_DYNAMO_ENDPOINT` /
    /// `ATOMR_IT_DYNAMO_ENDPOINT` (dynamodb-local).
    pub fn from_env() -> Self {
        let table =
            env::var("ATOMR_PERSISTENCE_DYNAMO_TABLE").unwrap_or_else(|_| "atomr_persistence".to_string());
        let endpoint = env::var("ATOMR_PERSISTENCE_DYNAMO_ENDPOINT")
            .or_else(|_| env::var("ATOMR_IT_DYNAMO_ENDPOINT"))
            .ok();
