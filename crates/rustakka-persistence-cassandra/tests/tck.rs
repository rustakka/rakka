//! Cassandra conformance. Requires `RUSTAKKA_IT_CASSANDRA_NODES` (comma
//! separated host:port list) to be set; tests skip otherwise.

use std::env;

use rustakka_persistence_cassandra::{CassandraConfig, CassandraJournal, CassandraSnapshotStore};
use rustakka_persistence_tck::{journal_suite, snapshot_round_trip, snapshot_suite};

fn it_nodes() -> Option<Vec<String>> {
    env::var("RUSTAKKA_IT_CASSANDRA_NODES")
        .ok()
...
async fn cassandra_journal_passes_tck() {
    let Some(nodes) = it_nodes() else {
        eprintln!("skip: RUSTAKKA_IT_CASSANDRA_NODES not set");
        return;
    };
