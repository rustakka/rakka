...
impl CassandraConfig {
    ...
    /// Env lookup: `RUSTAKKA_PERSISTENCE_CASSANDRA_NODES` (comma separated),
    /// `RUSTAKKA_IT_CASSANDRA_NODES`, `CASSANDRA_NODES`, dev fallback
    /// `127.0.0.1:9042`.
    pub fn from_env() -> Self {
        let nodes = env::var("RUSTAKKA_PERSISTENCE_CASSANDRA_NODES")
            .or_else(|_| env::var("RUSTAKKA_IT_CASSANDRA_NODES"))
            .or_else(|_| env::var("CASSANDRA_NODES"))
            .unwrap_or_else(|_| "127.0.0.1:9042".to_string());
        let keyspace = env::var("RUSTAKKA_PERSISTENCE_CASSANDRA_KEYSPACE")
