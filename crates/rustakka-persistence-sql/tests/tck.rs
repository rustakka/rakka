...
use rustakka_persistence::SnapshotStore as _;
use rustakka_persistence_sql::{SqlConfig, SqlJournal, SqlReadJournal, SqlSnapshotStore};
use rustakka_persistence_tck::{
...
async fn sqlite_read_journal_events_by_tag() {
    let j = new_journal().await;
    use rustakka_persistence::{Journal, PersistentRepr};
    let reprs = vec![
        PersistentRepr {
...
async fn config_from_env_defaults_to_memory() {
    ...
    std::env::remove_var("RUSTAKKA_PERSISTENCE_SQL_URL");
    std::env::remove_var("RUSTAKKA_IT_SQL_URL");
