//! Integration test wiring the cluster probe to live `rustakka-cluster`
//! `Gossip` / `MembershipState` values.

#![cfg(feature = "cluster")]

use rustakka_cluster::{Gossip, Member, MemberStatus};
use rustakka_core::actor::Address;
use rustakka_telemetry::cluster::{from_cluster_state, from_gossip, ClusterProbe};
