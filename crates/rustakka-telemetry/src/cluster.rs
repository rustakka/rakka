//! Cluster state probe — wraps `rustakka-cluster`'s `MembershipState` /
...
/// Convert a `rustakka-cluster` `MembershipState` into our serializable
/// `ClusterStateInfo`. Feature-gated because the cluster crate is optional.
#[cfg(feature = "cluster")]
pub fn from_cluster_state(state: &rustakka_cluster::MembershipState) -> ClusterStateInfo {
    use rustakka_cluster::ReachabilityStatus;
...
#[cfg(feature = "cluster")]
pub fn from_gossip(gossip: &rustakka_cluster::Gossip) -> ClusterStateInfo {
...
impl ClusterProbe {
    /// Convenience: update from a live `rustakka-cluster::Gossip`.
    #[cfg(feature = "cluster")]
    pub fn update_from_gossip(&self, gossip: &rustakka_cluster::Gossip) {
        self.update(from_gossip(gossip));
    }

    /// Convenience: update from a `rustakka-cluster::MembershipState`.
    #[cfg(feature = "cluster")]
    pub fn update_from_state(&self, state: &rustakka_cluster::MembershipState) {
