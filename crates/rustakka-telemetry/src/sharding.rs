...
/// Build a [`ShardRegionInfo`] from a live `rustakka-cluster-sharding`
/// region. Feature-gated.
#[cfg(feature = "sharding")]
pub fn region_info<E: rustakka_cluster_sharding::MessageExtractor>(
    region: &rustakka_cluster_sharding::ShardRegion<E>,
) -> ShardRegionInfo {
    ShardRegionInfo {
...
pub fn coordinator_allocations(
    coord: &rustakka_cluster_sharding::ShardCoordinator,
) -> Vec<(String, String)> {
    coord.allocations()
}
...
impl ShardingProbe {
    ...
    #[cfg(feature = "sharding")]
    pub fn refresh_from<E: rustakka_cluster_sharding::MessageExtractor>(
        &self,
        regions: &[&rustakka_cluster_sharding::ShardRegion<E>],
        coordinator: &rustakka_cluster_sharding::ShardCoordinator,
    ) {
