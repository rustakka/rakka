//! # rustakka-telemetry
//!
//! Optional probe surface for observing a running `rustakka` node.
//!
//! The crate is passive and opt-in. Construct a [`TelemetryExtension`] and
//! register it on an [`rustakka_core::actor::ActorSystem`] via
//! [`rustakka_core::actor::Extensions`]. Subsystems check for the extension
...
use rustakka_core::actor::ActorSystem;
