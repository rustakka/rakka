//! Prometheus exporter for `rustakka-telemetry`.
//!
...
mod tests {
    ...
    fn emits_expected_metrics_after_events() {
        let exp = PrometheusExporter::with_namespace("node-1", Some("rustakka")).unwrap();
        ...
        assert!(body.contains("rustakka_actors_spawned_total"));
        assert!(body.contains("rustakka_mailbox_depth"));
        assert!(body.contains("rustakka_dead_letters_total"));
        assert!(body.contains("rustakka_persistence_events_written_total"));
        assert!(body.contains("rustakka_persistence_last_sequence_nr"));
        assert!(body.contains("rustakka_cluster_member_events_total"));
