...
/// Populate the probe from a live `EndpointRegistry`. Creates an entry
/// for every remote address with state `"active"` and zeroed byte
/// counters; the remote layer is expected to call
/// [`RemoteProbe::record_inbound_bytes`] / `record_outbound_bytes` as
/// traffic flows.
#[cfg(feature = "remote")]
pub fn refresh_from_registry(
    probe: &RemoteProbe,
    registry: &rustakka_remote::EndpointRegistry,
) {
    use std::collections::HashSet;
    let live: HashSet<String> = registry.addresses().into_iter().collect();
    for addr in &live {
        if !probe.associations.contains_key(addr) {
            probe.record_association(addr, "active");
        }
    }
