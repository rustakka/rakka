...
//!
//! Exposes:
//! * `#[actor_msg]` attribute — adds `Debug` derive and rakka-friendly
...
pub fn derive_actor(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        #[::rakka_core::prelude::async_trait]
        impl #impl_generics ::rakka_core::prelude::Actor for #name #ty_generics #where_clause {
            type Msg = #msg_ty;

            async fn handle(
                &mut self,
                ctx: &mut ::rakka_core::prelude::Context<Self>,
                msg: Self::Msg,
            ) {
...
///
/// Generates an `impl rakka_core::actor::Actor` whose `handle` method
...
pub fn props(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ::rakka_core::actor::Props::create(move || #expr)
    };
    expanded.into()
}
