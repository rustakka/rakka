...
use rakka_core::actor::{Actor, ActorSystem, Context, LifecyclePhase, Props, Running, Starting, Stopping};
...
fn phase_marker_compile_time_witnesses() {
    fn _assert<A: Actor>(c: &mut Context<A>) {
        let _: Option<rakka_core::actor::TypedContext<'_, A, Starting>> = c.starting();
        let _: Option<rakka_core::actor::TypedContext<'_, A, Running>> = c.running();
        let _: Option<rakka_core::actor::TypedContext<'_, A, Stopping>> = c.stopping_view();
    }
}
