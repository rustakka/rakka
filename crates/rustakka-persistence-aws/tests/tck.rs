//! DynamoDB conformance. Requires `RUSTAKKA_IT_DYNAMO_ENDPOINT`
//! (e.g. pointing at dynamodb-local) to be set; tests skip otherwise.

use std::env;

use rustakka_persistence_aws::{DynamoConfig, DynamoJournal, DynamoSnapshotStore};
use rustakka_persistence_tck::{journal_suite, snapshot_round_trip, snapshot_suite};

fn it_endpoint() -> Option<String> {
    env::var("RUSTAKKA_IT_DYNAMO_ENDPOINT").ok()
}
...
async fn dynamo_journal_passes_tck() {
    let Some(endpoint) = it_endpoint() else {
        eprintln!("skip: RUSTAKKA_IT_DYNAMO_ENDPOINT not set");
        return;
    };
