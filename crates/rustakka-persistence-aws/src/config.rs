...
impl DynamoConfig {
    ...
    /// Env lookup: `RUSTAKKA_PERSISTENCE_DYNAMO_TABLE`, endpoint override
    /// via `RUSTAKKA_PERSISTENCE_DYNAMO_ENDPOINT` /
    /// `RUSTAKKA_IT_DYNAMO_ENDPOINT` (dynamodb-local).
    pub fn from_env() -> Self {
        let table = env::var("RUSTAKKA_PERSISTENCE_DYNAMO_TABLE")
            .unwrap_or_else(|_| "rustakka_persistence".to_string());
        let endpoint = env::var("RUSTAKKA_PERSISTENCE_DYNAMO_ENDPOINT")
            .or_else(|_| env::var("RUSTAKKA_IT_DYNAMO_ENDPOINT"))
            .ok();
