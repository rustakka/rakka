...
(`rustakka._native`) produced by [`pycore`](pycore/). The sibling
...
| Crate | Purpose |
|-------|---------|
| [`pycore`](pycore/) | **The actual bindings crate.** Exposes every subsystem as a submodule of `rustakka._native`. |
...
- Splitting into per-crate wheels is a downstream packaging decision
  and can happen later without touching the Python facade (`python/rustakka/`).
...
cargo check  -p rustakka-pycore
cargo clippy -p rustakka-pycore --no-deps
```
...
- Errors returned to Python use `crate::errors::map(e)` which wraps
  anything `Display` as `RustakkaError` or its subclasses.
