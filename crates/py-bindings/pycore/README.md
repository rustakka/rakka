...
PyO3 bindings for the `atomr` workspace. Compiles to
`atomr._native`, a single cdylib that hosts every subsystem as a
Python submodule.
...
cargo check -p atomr-pycore  # type-check only (no Python needed at link time thanks to abi3)
```
