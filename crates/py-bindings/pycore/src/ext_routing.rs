        ...
        match &self.logic {
                ...
                for (i, c) in self.children.iter().enumerate() {
                    if i == last {
                        ...
                        c.tell(PyMessage {
                            ...
                            reply: None,
                            sender_ref: msg.sender_ref.clone(),
                            hash: msg.hash,
                        });
                    } else {
            ...
            RouterLogicState::TailChopping { cursor, interval, within: _ } => {
                ...
                tokio::spawn(async move {
                    ...
                    secondary.tell(PyMessage {
                        payload: payload_for_retry,
                        reply: None,
                        sender_ref: None,
                        hash,
                    });
            ...
            RouterLogicState::ScatterGather { within: _ } => {
                ...
                for (i, c) in self.children.iter().enumerate() {
                    let env = if i == last {
                        PyMessage {
                            payload: msg.payload.clone_ref_gil(),
                            reply: None,
                            sender_ref: msg.sender_ref.clone(),
                            hash: msg.hash,
                        }
...
fn _backoff(
    ...
    let merged = PyProps {
        ...
        supervisor_strategy: inner.supervisor_strategy.clone(),
