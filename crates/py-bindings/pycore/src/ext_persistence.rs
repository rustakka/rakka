...
use rustakka_persistence::{InMemoryJournal, Journal, PersistentRepr};

use crate::errors;
use crate::runtime::runtime;

#[pyclass(name = "InMemoryJournal", module = "rustakka._native.persistence")]
...
impl PyInMemoryJournal {
    #[new]
    fn new() -> Self { Self { inner: InMemoryJournal::new() } }
    ...
    fn replay<'py>(&self, py: Python<'py>, pid: String) -> PyResult<Bound<'py, PyList>> {
        let inner = self.inner.clone();
        let rt = runtime();
        let reprs = py.allow_threads(|| rt.block_on(async move {
            inner
                .replay_messages(&pid, 1, u64::MAX, u64::MAX)
                .await
                .map_err(errors::map)
        }))?;
    ...
    fn highest_sequence_nr(&self, py: Python<'_>, pid: String) -> PyResult<u64> {
        ...
        let rt = runtime();
        py.allow_threads(|| rt.block_on(async move {
            inner.highest_sequence_nr(&pid, 0).await.map_err(errors::map)
        }))
    }
}
