...
impl PyInMemoryLease {
    #[new]
    fn new() -> Self {
        Self { inner: InMemoryLease::new() }
    }

    fn acquire(&self, py: Python<'_>, owner: String) -> PyResult<bool> {
        let inner = self.inner.clone();
        let rt = runtime();
        py.allow_threads(|| rt.block_on(async move { inner.acquire(&owner).await.map_err(errors::map) }))
    }

    fn release(&self, py: Python<'_>, owner: String) -> PyResult<()> {
        let inner = self.inner.clone();
        let rt = runtime();
        py.allow_threads(|| rt.block_on(async move { inner.release(&owner).await.map_err(errors::map) }))
    }
