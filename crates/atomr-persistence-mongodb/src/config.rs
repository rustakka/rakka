...
impl MongoConfig {
    ...
    /// Env lookup: `ATOMR_PERSISTENCE_MONGO_URL`, `ATOMR_IT_MONGO_URL`,
    /// `MONGODB_URL`, dev fallback `mongodb://127.0.0.1:27017`.
    pub fn from_env() -> Self {
        let url = env::var("ATOMR_PERSISTENCE_MONGO_URL")
            .or_else(|_| env::var("ATOMR_IT_MONGO_URL"))
            .or_else(|_| env::var("MONGODB_URL"))
            .unwrap_or_else(|_| "mongodb://127.0.0.1:27017".to_string());
        let db = env::var("ATOMR_PERSISTENCE_MONGO_DB").unwrap_or_else(|_| "atomr".into());
        Self::new(url, db)
    }
}
...
mod tests {
    ...
    #[test]
    fn defaults() {
        let cfg = MongoConfig::new("mongodb://x", "atomr");
