...
impl RedisConfig {
    pub fn new(url: impl Into<String>) -> Self {
        Self { url: url.into(), key_prefix: "rakka".into(), pool_size: 4 }
    }
    ...
    /// Env lookup order: `RAKKA_PERSISTENCE_REDIS_URL`, `RAKKA_IT_REDIS_URL`,
    /// `REDIS_URL`, then dev fallback `redis://127.0.0.1:6379`.
    pub fn from_env() -> Self {
        let url = env::var("RAKKA_PERSISTENCE_REDIS_URL")
...
mod tests {
    ...
    #[test]
    fn from_env_default() {
        std::env::remove_var("RAKKA_PERSISTENCE_REDIS_URL");
        std::env::remove_var("RAKKA_IT_REDIS_URL");
