...
use rakka_telemetry::dto::{ActorStatus, DeadLetterRecord};
use rakka_telemetry::exporters::config::{ExportersConfig, PrometheusConfig};
...
async fn metrics_endpoint_renders_registry() {
    ...
    assert!(body.contains("rakka_actors_spawned_total"), "body: {body}");
    assert!(body.contains("rakka_dead_letters_total"), "body: {body}");
