...
React + Vite + TypeScript single-page app for the atomr dashboard
backend (`crates/atomr-dashboard`).
...
```bash
cargo run -p atomr-dashboard --features bin --bin atomr-dashboard \
  -- --bind 127.0.0.1:9100
```
...
```bash
pnpm build
cargo build -p atomr-dashboard --features embed-ui
```
