//! Thin adapter — instantiates the `atomr-telemetry` exporters the
...
use atomr_telemetry::exporters::config::ExportersConfig;
...
pub struct ExporterHandles {
    ...
    pub prometheus: Option<Arc<atomr_telemetry::exporters::prometheus::PrometheusExporter>>,
}
...
fn apply_prometheus(
    telemetry: &Arc<TelemetryExtension>,
    cfg: &atomr_telemetry::exporters::config::PrometheusConfig,
    handles: &mut ExporterHandles,
) -> Result<(), ServerError> {
    let exp = atomr_telemetry::exporters::prometheus::PrometheusExporter::with_namespace(
...
fn apply_prometheus(
    ...
    _cfg: &atomr_telemetry::exporters::config::PrometheusConfig,
...
fn apply_otlp(
    telemetry: &Arc<TelemetryExtension>,
    cfg: &atomr_telemetry::exporters::config::OtlpConfig,
) -> Result<(), ServerError> {
    let exporter =
        atomr_telemetry::exporters::otel::OtelExporter::new_with_node(cfg.clone(), telemetry.node.clone())
...
fn apply_otlp(
    _telemetry: &Arc<TelemetryExtension>,
    _cfg: &atomr_telemetry::exporters::config::OtlpConfig,
) -> Result<(), ServerError> {
