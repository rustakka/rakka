...
impl RedisConfig {
    pub fn new(url: impl Into<String>) -> Self {
        Self { url: url.into(), key_prefix: "atomr".into(), pool_size: 4 }
    }
    ...
    /// Env lookup order: `ATOMR_PERSISTENCE_REDIS_URL`, `ATOMR_IT_REDIS_URL`,
    /// `REDIS_URL`, then dev fallback `redis://127.0.0.1:6379`.
    pub fn from_env() -> Self {
        let url = env::var("ATOMR_PERSISTENCE_REDIS_URL")
            .or_else(|_| env::var("ATOMR_IT_REDIS_URL"))
            .or_else(|_| env::var("REDIS_URL"))
            .unwrap_or_else(|_| "redis://127.0.0.1:6379".to_string());
        let prefix = env::var("ATOMR_PERSISTENCE_REDIS_PREFIX").unwrap_or_else(|_| "atomr".into());
...
mod tests {
    ...
    #[test]
    fn from_env_default() {
        std::env::remove_var("ATOMR_PERSISTENCE_REDIS_URL");
        std::env::remove_var("ATOMR_IT_REDIS_URL");
