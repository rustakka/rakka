...
use rustakka_telemetry::dto::{ActorStatus, DeadLetterRecord};
use rustakka_telemetry::exporters::config::{ExportersConfig, PrometheusConfig};
...
async fn metrics_endpoint_renders_registry() {
    ...
    assert!(body.contains("rustakka_actors_spawned_total"), "body: {body}");
    assert!(body.contains("rustakka_dead_letters_total"), "body: {body}");
