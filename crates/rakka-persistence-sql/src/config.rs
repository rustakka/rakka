...
impl SqlConfig {
    ...
    ///
    /// Lookup order:
    /// 1. `RAKKA_PERSISTENCE_SQL_URL` (any environment, explicit override).
    /// 2. `RAKKA_IT_SQL_URL` (test integration).
    /// 3. `DATABASE_URL` (prod conventions).
    /// 4. Dev fallback: `sqlite::memory:`.
    pub fn from_env() -> Self {
        let url = env::var("RAKKA_PERSISTENCE_SQL_URL")
            .or_else(|_| env::var("RAKKA_IT_SQL_URL"))
            .or_else(|_| env::var("DATABASE_URL"))
            .unwrap_or_else(|_| "sqlite::memory:".to_string());
        let auto_migrate = env::var("RAKKA_PERSISTENCE_SQL_AUTO_MIGRATE")
