# **Rustakka Persistence Providers Plan**
...
## **2\. Implementation Plan for rustakka**
...
Set up a Cargo workspace within the rustakka repository (or a dedicated rustakka-persistence repo) to manage the providers:

* rustakka-persistence (Core traits: AsyncWriteJournal, SnapshotStore, ReadJournal)  
* rustakka-persistence-sql (Unified SQL provider for Postgres, MySQL, SQLite, MSSQL)  
* rustakka-persistence-redis  
* rustakka-persistence-mongodb  
* rustakka-persistence-cassandra
...
* **Async Runtime:** tokio (assuming rustakka uses it).  
...
1. **Phase 1: Core Definitions & In-Memory**  
   ...
   * Implement rustakka-persistence-memory for testing.  
2. **Phase 2: Unified SQL Provider (rustakka-persistence-sql)**  
...
3. **Phase 3: High-Performance NoSQL (redis & mongodb)**  
   * Implement rustakka-persistence-redis (prioritize due to high throughput use cases).  
   * Implement rustakka-persistence-mongodb (JSON/BSON document storage mapping).  
  ...
  `publish:`  
    ...
    `steps:`  
      ...
      `- name: Cargo Publish Core`  
        ...
        `run: cargo publish -p rustakka-persistence`

      `- name: Cargo Publish SQL`  
        `env:`  
          `CARGO_REGISTRY_TOKEN: ${{ secrets.CRATES_IO_TOKEN }}`  
        `run: cargo publish -p rustakka-persistence-sql`
...
3. **Draft a Release on GitHub:**  
   * Go to your rustakka GitHub repo \-\> **Releases** \-\> **Draft a new release**.  
