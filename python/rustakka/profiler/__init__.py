...
Mirrors the ``rustakka-profiler`` Rust binary — same scenarios
...
.. code:: bash
   ...
   python -m rustakka.profiler --scenario all --messages 2000 --format md
   python -m rustakka.profiler --format json --output report.json

Or programmatically:

.. code:: python

   from rustakka.profiler import run
   report = run()
...
def run(
    ...
    report = ProfilerReport(version=rustakka.__version__, host=host_tag())
    system = rustakka.ActorSystem.create_blocking(system_name)
    try:
        for s in scenarios:
...
def _build_cli() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python -m rustakka.profiler",
