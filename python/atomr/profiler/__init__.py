...
Mirrors the ``atomr-profiler`` Rust binary — same scenarios
...
.. code:: bash
   ...
   python -m atomr.profiler --scenario all --messages 2000 --format md
   python -m atomr.profiler --format json --output report.json
...
def run(
    ...
    report = ProfilerReport(version=atomr.__version__, host=host_tag())
    system = atomr.ActorSystem.create_blocking(system_name)
    try:
        for s in scenarios:
