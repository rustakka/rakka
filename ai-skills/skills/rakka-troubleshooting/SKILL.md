---
name: rakka-troubleshooting
description: Use when diagnosing failures in a project that depends on rakka. Covers compile-time errors (missing feature flags, wrong prelude), runtime issues (mailbox stalls, restart loops, ask timeouts), and cluster-level problems (split-brain, unreachable members). Triggers on rakka error messages, missing-symbol errors, hangs, or repeated restarts.
---

# Troubleshooting rakka

A diagnostic checklist organized by symptom. For deep dives into a
subsystem, hand off to the matching skill (`rakka-cluster`,
`rakka-persistence`, `rakka-python`).

## Compile-time

### "cannot find type `X` in crate `rakka`"
...
|---|---|---|
| `Actor`, `ActorRef`, `Props`, `Context` | (always available, in `rakka::prelude`) | `rakka::core` |
| `TestKit`, `TestProbe` | `testkit` | `rakka::testkit` |
| `Cluster*` | `cluster` (or `cluster-tools` / `cluster-sharding`) | `rakka::cluster*` |
| `PersistentActor`, `Eventsourced` | `persistence` | `rakka::persistence` |
| `Source`, `Flow`, `Sink` | `streams` | `rakka::streams` |
| `Telemetry` exporters | `telemetry` | `rakka::telemetry` |

```toml
rakka = { package = "rakka-rs", version = "0.2", features = ["cluster", "persistence"] }
```

If you depend on a subsystem crate directly (e.g. `rakka-persistence`)
...
```rust
use rakka::prelude::*;     // re-exports `async_trait`
...
- For remote/cluster refs, check that delivery semantics are what you
  expect: rakka-remote provides at-most-once by default; opt into
  reliable delivery if you need it.

## Cluster

For full cluster troubleshooting, hand off to `rakka-cluster`. Quick
checks:
...
- **Split-brain.** Configure an SBR (split-brain resolver). See
  `crates/rakka-cluster/src/sbr.rs` and `docs/architecture.md`.
- **Sharding stuck rebalancing.** Coordinator persistence broken — see
  `rakka-persistence` skill.
...
The dashboard (`rakka-dashboard`, feature `dashboard`) gives a live
...
- `crates/rakka-core/src/supervision.rs` — supervision strategies
