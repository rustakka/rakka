---
name: rakka-actor-design
description: Use when authoring or modifying an actor in a project that depends on rakka. Covers Msg type design, the Actor trait's lifecycle hooks, ask vs tell, supervision strategies, and FSM patterns. Triggers on writing or editing an `impl Actor`, choosing a Msg enum, picking a supervision strategy, or wiring `Props`/`ActorSystem`.
---

# Authoring rakka actors

This skill helps you write idiomatic actors against `rakka` (umbrella
crate `rakka-rs` on crates.io, imported as `rakka`).

## Mental model

A rakka actor is **state + behavior + a typed mailbox**. Each actor:
...
Defined in `rakka_core::actor::traits`; available from the prelude.
...
hand-written FSM pattern in `rakka_core::actor`) over branching on a
state field inside `handle`. See `crates/rakka-core/src/fsm_macro.rs`
and `crates/rakka-persistence/src/persistent_fsm.rs` for the pattern.

## When to reach beyond core

| You need… | Reach for… |
|---|---|
| State that survives restart (event sourcing) | `rakka-persistence` — see `rakka-persistence` skill |
| Cross-process / cross-host messaging | `rakka-remote` |
| Membership, sharding, pub/sub | `rakka-cluster*` — see `rakka-cluster` skill |
| Reactive pipelines | `rakka-streams` |
| Deterministic tests | `rakka-testkit` — see `rakka-testing` skill |

## Canonical references

- `crates/rakka-core/src/actor/traits.rs` — `Actor` trait definition
- `crates/rakka-core/src/supervision.rs` — strategies and directives
- `crates/rakka-core/src/lib.rs` — prelude contents
