...
|---|---|---|
| Cross-process messaging only | `remote` | `rakka-remote` |
| Membership, gossip, reachability | `cluster` | `rakka-cluster` |
| Singleton, distributed pub/sub, cluster client | `cluster-tools` | `rakka-cluster-tools` |
| Sharded entities (rebalance, remember-entities) | `cluster-sharding` | `rakka-cluster-sharding` |
| Adaptive load balancing | `cluster-metrics` | `rakka-cluster-metrics` |
| CRDTs (`Replicator`) | `distributed-data` | `rakka-distributed-data` |
...
`ClusterSingleton` (in `rakka-cluster-tools`) elects exactly one
...
`DistributedPubSub` (from `rakka-cluster-tools`) gives you
...
maps), use `rakka-distributed-data::Replicator`. It exposes the usual
CRDTs (G-Counter, OR-Set, LWW-Register, etc.) and replicates writes
under tunable consistency (`Local`, `Majority`, `All`).

## Split-brain

Always configure a split-brain resolver in production. rakka ships
several strategies (`KeepMajority`, `StaticQuorum`, `KeepOldest`,
`DownAllWhenUnstable`). See `crates/rakka-cluster/src/sbr.rs` and
`docs/architecture.md`.

## Observability

The dashboard (`rakka-dashboard`) and telemetry exporters are the
...
- `examples/sharded-keyvalue/` — `ShardRegion`, `MessageExtractor`,
  `PassivationTracker`
- `crates/rakka-cluster/src/lib.rs` — membership API
- `crates/rakka-cluster/src/sbr.rs` — split-brain resolvers
- `crates/rakka-cluster-sharding/src/lib.rs` — sharding API
- `crates/rakka-cluster-tools/src/lib.rs` — singleton, pub/sub
