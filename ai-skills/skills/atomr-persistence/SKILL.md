---
name: atomr-persistence
description: Use when adding event sourcing to an actor in a project that depends on atomr. Covers the Eventsourced trait, journals, snapshots, recovery, RecoveryPermitter, AsyncSnapshotter, and choosing a storage adapter (sql / redis / mongodb / cassandra / aws / azure). Triggers when implementing PersistentActor or Eventsourced, configuring a Journal/SnapshotStore, or wiring a backend.
---

# Event sourcing with atomr-persistence

`atomr-persistence` provides the plugin traits (`Journal`,
...
|---|---|---|
| `atomr-persistence-sql` | SQLite, Postgres, MySQL, MSSQL (via `sqlx`) | `sqlite` |
| `atomr-persistence-redis` | Redis / KeyDB (via `fred`) | — |
| `atomr-persistence-mongodb` | MongoDB | — |
| `atomr-persistence-cassandra` | Cassandra / ScyllaDB | — |
| `atomr-persistence-aws` | DynamoDB single-table | — |
| `atomr-persistence-azure` | Azure Table Storage | — |

```toml
[dependencies]
atomr = { version = "0.2", features = ["persistence"] }
atomr-persistence-sql = { version = "0.1", default-features = false, features = ["postgres"] }
```

Every adapter is validated against `atomr-persistence-tck`'s shared
...
use atomr_persistence::{Eventsourced, EventsourcedError};
...
```rust
use atomr_persistence::{AsyncSnapshotter, SnapshotPolicy, InMemorySnapshotStore};
...
Your backend must satisfy `atomr-persistence-tck`'s suites:
...
A passing TCK run is the bar for "compatible with atomr-persistence".

## Canonical references

- `examples/event-sourced-counter/` — `Eventsourced` + snapshots
- `crates/atomr-persistence/src/eventsourced.rs` — trait definition
- `crates/atomr-persistence/src/recovery_permitter.rs` — bounded recovery
