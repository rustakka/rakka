---
name: rakka-testing
description: Use when writing tests against rakka actors using rakka-testkit. Covers TestKit, TestProbe (expect_msg / receive_n / expect_no_msg / expect_msg_pf), and the virtual-time TestScheduler. Triggers when authoring `#[tokio::test]` cases that exercise actors, mailboxes, or scheduled timers.
---

# Testing rakka actors

`rakka-testkit` provides the deterministic scaffolding that lets you
...
```toml
[dev-dependencies]
rakka = { package = "rakka-rs", version = "0.2", features = ["testkit"] }
# or directly:
rakka-testkit = "0.1"
```
...
use rakka::prelude::*;
use rakka::testkit::{TestKit, TestProbe};
...
```rust
use std::time::Duration;
use rakka::testkit::TestScheduler;
...
`rakka_testkit::MultiNodeSpec`. It boots multiple nodes in one test and
exposes barriers and event filters. See the integration tests in
`crates/rakka-cluster/tests/` and `crates/rakka-persistence-tck/` for
working examples.

## Conformance suites

If you implement a `Journal` or `SnapshotStore`, run it through
`rakka-persistence-tck`'s `journal_suite`, `journal_tag_suite`, and
`snapshot_suite`. They are the source of truth for what "compatible
with rakka-persistence" means.

## Canonical references

- `crates/rakka-testkit/src/probe.rs` — `TestProbe` API
- `crates/rakka-testkit/src/test_kit.rs` — `TestKit::probe`
- `crates/rakka-testkit/src/test_scheduler.rs` — `TestScheduler`
- `crates/rakka-testkit/src/multinode.rs` — `MultiNodeSpec`
